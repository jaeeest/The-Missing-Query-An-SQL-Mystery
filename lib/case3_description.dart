...
class ProfileFlipCard extends StatelessWidget {
  ...
  List<String> _getFramesForPage(int pageNumber) {
    ...
    return List.generate(
      frameCount,
      (index) => 'assets/Case3/character_profile$pageNumber/${index + 1}.png',
    );
  }
...
// --- MAIN SCREEN ---
class CaseDescription3 extends StatefulWidget {
  const CaseDescription3({super.key});

  @override
  State<CaseDescription3> createState() => _CaseDescription3State();
}

class _CaseDescription3State extends State<CaseDescription3>
  ...
  final List<String> _descriptions = [
    "A sudden report came to the Baldwin City Police. At 2:14 PM, a neighbor reported loud shouting and glass breaking at a suburban home in the South Ward. At 2:17 PM, a conveyor belt at the Baldwin City Waste Management Corp. ground to a halt when an employee spotted a human hand.",
    "Officer Johnson was the first on the scene at the house. He found a birthday cake, a dead woman, and a blood-stained dinner table. But before he could call in the discovery, a figure emerged from the shadows. A single shot rang out, hitting Johnson in the chest. As he bled out on the chest, the killer fled.",
    "As the lead investigator, you've been brought in to solve the crime. Your job is to query the logs, cross-reference the suspects, and find the connection between a birthday dinner gone wrong and a human hand found in the city.",
  ];
  ...
  void _handleNext() {
    ...
    Navigator.push(context, fadeSlideRoute(const CaseMap3()));
  }
      ...
      child: Container(
        ...
        child: Stack(
          children: [
            Positioned.fill(
              child: Image.asset(
                _isBoardVisible ? 'assets/Case3/case_board3.png' : 'assets/case_bg.png',
                fit: BoxFit.fill,
              ),
            ),
            if (!_isBoardVisible)
